...
for (std::string line; getline(infile, line);) {		
	...
	if(val.compare("cam") == 0)
	{
		...
		scene.camera_t(): eye(ex, ey, ez),  //takes place of camera.h constructor
			UL(ulx, uly, ulz), UR(urx, ury, urz), 
			LL(llx, lly, llz), LR(lrx, lry, lrz)
		{};
	}
	...
	/////
		...
		scene.light_t(true, px, py, pz, r, g, b);
	}
	else if(val.compare("ltd") == 0) {
		float dx dy dz r g b;
		ss >> dx >> dy >> dz >> r >> g >> b;

		scene.light_t(true, dx, dy, dz, r, g, b);
	}
